...
func Run(path string, refresh time.Duration) {
	...
	cfg := &config.Config{
		...
		InformerRefresh:      refresh * time.Second,
	}
...
func run(c *config.CompletedConfig, stopCh <-chan struct{}) error {
	...
	if c.SecureServing != nil {
		unsecuredMux = NewBaseHandler(healthHandler)
	...
	startController := func() {
		...
		addonInformerFactory := informers.NewSharedInformerFactory(addonClient, c.InformerRefresh)
